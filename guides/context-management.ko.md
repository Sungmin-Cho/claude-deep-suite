...
이 문서는 **compaction**, **output offloading**, **full context reset** 의 세 가지 — 장시간 실행 작업에서 agent 추론 품질을 보존하는 핵심 레버 — 에 대한 suite 차원 정책을 정리한다. Addy Osmani 와 Anthropic 팀이 long-running agent 의 반복적인 실패 모드로 명시한 영역; Deep Suite 는 10개 플러그인이 일관되게 동작하도록 정책을 여기로 모은다.
