...
**A coverage test that derives its expectation from what it is checking cannot fail.** One guard proved its scan set complete by looping over the same directory list the scan set was built from. Shrinking that list shrank the loop too, so dropping `agents` killed nothing while both agent definitions silently left coverage. The expectation has to come from a different authority than the subject — here, the git-derived file index — and the discriminator has to be a property rather than a list: *nested* markdown under a shipped directory is an instruction surface by construction, root-level markdown is repository metadata. Ask where your expected set comes from; if it is the thing under test, the test is a mirror.

**A singleton is an enumeration of one.** A rule keyed on *the* anchor spelling silenced itself on any other: the guard flagged a bare `sensors/detect.js` and passed `$PLUGIN_DIR/sensors/detect.js`, an undefined variable that expands to nothing — and two shipped skills were using it. The find came from a sibling repo turning up `${CLAUDE_PLUGIN_DIR}`, `_DIR` rather than `_ROOT`, which no list in the family would have held. The repair is the usual one, a property instead of a name: a variable-rooted path whose tail resolves in the shipped index is rooting at this plugin, whatever the variable is called — measured, zero overlap with the legitimate workspace roots that outnumber it 195 to 4. And derive the accepted names from the runtime's own env reads rather than writing them down: the first attempt assumed one anchor, reported the runtime's second supported spelling as rogue, and three correct lines were "fixed" before the premise was checked.
