...
| 안티패턴 | 왜 나쁜가 |
|---|---|
| LLM을 호출하는 hook | 비결정론적; 같은 프로젝트의 이전 출력을 LLM이 다시 먹으면 echo chamber 위험. |
...
| 광범위한 matcher (`*`) + per-call 부작용 | 세션 전체에 잡음을 증폭. matcher를 좁히고 (`Bash\|Write\|Edit`) 안쪽 `if`도 같이. |
...
| 재귀 삭제 | `Bash(rm -rf /*)`, `Bash(rm -rf ~/* )`, `Bash(rm -rf .*)` | 회복 불가능한 파국. |
| SQL drop / truncate | `Bash(* DROP TABLE *)`, `Bash(* TRUNCATE *)` | 운영 데이터 손실. |
| Kubectl 파괴적 | `Bash(kubectl delete *)`, `Bash(kubectl drain *)` | 공유 인프라 영향. |

Denylist의 목적은 망라 (exhaustive) 가 아니라, *흔한* foot-gun이 명시 override를 요구하도록 만드는 것.
