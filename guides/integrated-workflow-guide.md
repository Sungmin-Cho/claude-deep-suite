...
> Reflects **deep-work v6.6.0**, **deep-review v1.4.0**, **deep-evolve v3.3.0**, **deep-docs v1.2.0**, **deep-wiki v1.5.0**, **deep-dashboard v1.3.1** — all 6 plugins adopt the M3 common artifact envelope (cross-plugin `run_id` chain + envelope-aware payload validation against the payload registry under `schemas/payload-registry/`). deep-dashboard v1.3.1 closes M4 with suite-level telemetry (16 metrics, JSONL time-series, markdown trend report, optional OTLP export); deep-work v6.6.0 + deep-evolve v3.3.0 ship M5.7 producer-side adoption (handoff + compaction-state emit) closing the data-flow for 3 M5-activated metrics (handoff roundtrip + compaction frequency/preserved-ratio). See `docs/envelope-migration.md` for envelope schema + migration guide.
