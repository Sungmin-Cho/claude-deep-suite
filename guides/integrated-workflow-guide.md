...
> Reflects **deep-work v6.4.0** (Team/Solo subagent delegation for Research/Implement + post-hoc receipt verification; Phase Exit Gates from v6.3.1; Phase 5 Integrate from v6.3.0), **deep-review v1.3.4** (Phase 6 subagent delegation from v1.3.3 + v1.3.4 trust-boundary hardening: staged rename precision, allowlist bypass block via pre-existing outside content hash, dirty recovery index/worktree sync, tracked-deleted WIP preservation, macOS bash 3.2 compatibility via TSV temp files, ubuntu + macOS CI matrix; Stage 5.5 recurring findings, entropy retained), **deep-evolve v3.0.0** (AAR-inspired evidence-rich hill-climbing: idea-category entropy tracking, legibility gate, shortcut detector, diagnose-retry — all gated on `session.yaml.deep_evolve_version`; v2.2.2 sessions remain supported via soft migration), **deep-docs v1.1.0**, **deep-wiki v1.1.2** (subagent delegation for page I/O; structured manifest with per-source provenance and ingest-time content hashing), **deep-dashboard v1.1.1**.
