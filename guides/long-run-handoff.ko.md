...
> Wrapper 는 `envelope.schema.name = envelope.artifact_kind` 으로 자동 설정 (`scripts/wrap-artifact.js` 참조). 따라서 `--artifact-kind handoff` 만 넘기면 `envelope.schema.name = "handoff"` 도 같이 매치 — receiver 의 identity-triplet check (§4.2) 가 성공한다.
