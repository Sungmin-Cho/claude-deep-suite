...
> 반영 버전: **deep-work v6.4.2** (Profile schema v3 + `interactive_each_session` + session recommender sub-agent + `--no-ask`/`--recommender`/`--no-recommender` 플래그 + 알림 시스템 제거(breaking); Team/Solo subagent 위임은 v6.4.1; Phase Exit Gates는 v6.3.1; Phase 5 Integrate는 v6.3.0 신설), **deep-review v1.3.4** (v1.3.3의 Phase 6 서브에이전트 위임 + v1.3.4 trust-boundary hardening: staged rename precision, pre-existing outside content hash로 allowlist 우회 차단, dirty recovery의 index/worktree 동기 복원, tracked-deleted WIP 보존, macOS bash 3.2 호환 — TSV temp file이 `declare -A` 대체, ubuntu + macOS CI matrix; Stage 5.5 recurring findings, entropy 유지), **deep-evolve v3.1.1** (v3.1 virtual parallel N-seed 탐색, 적응형 스케줄러, cross-seed forum, cascade synthesis; v3.1.1은 stdout metric 실패 처리, sealed prepare read/write guard, scheduler journal 계약, 패키징 범위를 강화; v3.0.x 세션은 VERSION_TIER 라우팅으로 지원), **deep-docs v1.1.0**, **deep-wiki v1.4.0** (A5 단일 소스 페이지 단위 fanout — `wiki-synthesizer mode="analysis"`가 `page_plan` + sub-threshold `inline_bodies` emit; `wiki-page-writer` worker가 페이지 본문 병렬 생성 (`tools: []`); main이 lock 아래 aggregate, mandatory C3 concurrency check; 295페이지 wiki 초기 dogfood에서 Claude Code 런타임 concurrent-subagent cap ~3 환경 기준 총 ~17분 측정 (v1.3.0 ~15분 baseline 대비; ≤5분 목표는 v1.4.1 B1+B3 정량 측정으로 보류; M1 trust-boundary 실증 — Track C synthesizer agent split 우선순위 격상). Karpathy "10–15 page touches per source" 속성 보존. 신규 `partial_fail` sentinel (Step 1.5 cascading + `partial-fail-recovery` repair_reason); 선택적 `<wiki>/.wiki-meta/.config.json` (`a5_fanout_threshold`, `a5_worker_timeout_sec`); Step 7.7.A-F 실패 처리 + R4 metadata pipeline recovery 포함; 다중 소스 A4 경로는 v1.3.0과 byte-identical. 이전 line: v1.3.0 A4 다중 소스 synthesizer fanout (Approach B) + 훅 YAML 파서 확장; v1.2.x throughput + lint 강화 + ingest-repair 자가복구; v1.1.4 `source_hashes` 정규화 + `.pending-scan` promotion regression guard; v1.1.3 `wiki-synthesizer` 내부 parallel tool dispatch; v1.1.2 페이지 I/O subagent 위임), **deep-dashboard v1.1.1**.
