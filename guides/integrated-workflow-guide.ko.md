...
> 반영 버전: **deep-work v6.4.2** (Profile schema v3 + `interactive_each_session` + session recommender sub-agent + `--no-ask`/`--recommender`/`--no-recommender` 플래그 + 알림 시스템 제거(breaking); Team/Solo subagent 위임은 v6.4.1; Phase Exit Gates는 v6.3.1; Phase 5 Integrate는 v6.3.0 신설), **deep-review v1.3.4** (v1.3.3의 Phase 6 서브에이전트 위임 + v1.3.4 trust-boundary hardening: staged rename precision, pre-existing outside content hash로 allowlist 우회 차단, dirty recovery의 index/worktree 동기 복원, tracked-deleted WIP 보존, macOS bash 3.2 호환 — TSV temp file이 `declare -A` 대체, ubuntu + macOS CI matrix; Stage 5.5 recurring findings, entropy 유지), **deep-evolve v3.1.1** (v3.1 virtual parallel N-seed 탐색, 적응형 스케줄러, cross-seed forum, cascade synthesis; v3.1.1은 stdout metric 실패 처리, sealed prepare read/write guard, scheduler journal 계약, 패키징 범위를 강화; v3.0.x 세션은 VERSION_TIER 라우팅으로 지원), **deep-docs v1.1.0**, **deep-wiki v1.4.1** (Track C synthesizer 에이전트 분리로 trust-boundary 폐쇄 — 단일 `wiki-synthesizer`를 3개 분리 에이전트 `wiki-synthesizer-{analysis,worker,inline}`로 교체; active 에이전트 (analysis + worker)는 `tools: [Read, Glob, Grep, WebFetch]` — Write 물리적 제거로 tool-level M1 폐쇄 (v1.4.0 dogfood 실증 실패 모드 — 14 워커 중 2개가 계약 외부에 file write — 닫힘); qualified namespace routing (V-0 실증: `deep-wiki:wiki-X` resolve, unqualified는 error — Step 7.6.A에 `general-purpose` fallback 금지 명시 코멘트 추가); 신규 `scripts/lint-agent-tools.sh` Bash 3.2 portable manifest + `_post_dispatch_dirty_scan()` 3개 site (WIKI_TEST_MODE=1 gated); inline 파일은 DORMANT (`status:dormant` + `last_known_active:v1.3.0` + `contract_frozen_at:a9966c7` rot-mitigation 헤더). Layered defense-in-depth (NOT comprehensive enforcement): L1 (V-0는 런타임 메타데이터 API 없이 best-effort; Track C v2는 런타임 API가 메타데이터 노출을 지원할 때까지 보류) + L2 (§3.9는 wiki root scope만 — v1.4.0 실증 실패 모드인 off-root /tmp write 미감지; process-level sandboxing은 v1.5.0+로 보류) CHANGELOG에 honest disclosure. 마이그레이션: BREAKING — 외부 caller는 qualified `deep-wiki:wiki-synthesizer-{inline|analysis|worker}`로 전환 필수, Option B per compat shim 없음. A5 단일 소스 page-fanout + A4 다중 소스 경로는 v1.4.0에서 구조적 보존 (NOT byte-identical: split-agent dispatch shape 변경). 이전 line: v1.4.0 A5 단일 소스 페이지 단위 fanout (`wiki-synthesizer mode="analysis"` + `wiki-page-writer` worker); v1.3.0 A4 다중 소스 synthesizer fanout (Approach B) + 훅 YAML 파서 확장; v1.2.x throughput + lint 강화 + ingest-repair 자가복구; v1.1.4 `source_hashes` 정규화 + `.pending-scan` promotion regression guard; v1.1.3 `wiki-synthesizer` 내부 parallel tool dispatch; v1.1.2 페이지 I/O subagent 위임), **deep-dashboard v1.1.1**.
