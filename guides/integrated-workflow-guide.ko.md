...
> 반영 버전: **deep-work v6.4.2** (Profile schema v3 + `interactive_each_session` + session recommender sub-agent + `--no-ask`/`--recommender`/`--no-recommender` 플래그 + 알림 시스템 제거(breaking); Team/Solo subagent 위임은 v6.4.1; Phase Exit Gates는 v6.3.1; Phase 5 Integrate는 v6.3.0 신설), **deep-review v1.3.4** (v1.3.3의 Phase 6 서브에이전트 위임 + v1.3.4 trust-boundary hardening: staged rename precision, pre-existing outside content hash로 allowlist 우회 차단, dirty recovery의 index/worktree 동기 복원, tracked-deleted WIP 보존, macOS bash 3.2 호환 — TSV temp file이 `declare -A` 대체, ubuntu + macOS CI matrix; Stage 5.5 recurring findings, entropy 유지), **deep-evolve v3.1.1** (v3.1 virtual parallel N-seed 탐색, 적응형 스케줄러, cross-seed forum, cascade synthesis; v3.1.1은 stdout metric 실패 처리, sealed prepare read/write guard, scheduler journal 계약, 패키징 범위를 강화; v3.0.x 세션은 VERSION_TIER 라우팅으로 지원), **deep-docs v1.1.0**, **deep-wiki v1.1.4** (v1.1.2의 페이지 I/O subagent 위임 위에, v1.1.3에서 `wiki-synthesizer` 내부 각 phase의 tool call을 한 메시지에 병렬 발행하는 가이드 추가 — 멀티 페이지 ingest의 ~3N 순차 round-trip 지연 제거. v1.1.4는 `source_hashes` 정규화 — agent에 해싱 수단이 없을 때 main이 origin에서 post-hoc sha256 재계산해 `sources/*.yaml:content_hash`가 항상 실제 digest를 반영하도록 함 — 및 `.pending-scan → .last-scan` promotion regression guard — 버전 전환 / 수동 파일 변경으로 인한 stale `.pending-scan` 상황에서 `.last-scan` 역행 차단), **deep-dashboard v1.1.1**.
