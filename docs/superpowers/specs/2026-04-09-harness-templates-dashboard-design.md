...
감지 순서: specific → generic. 복수 매칭 시 가장 specific한 것 우선 (nextjs-app > react-spa > generic).
...
```json
{
  ...
  "fitness_defaults": {
    "rules": [
      {
        "name": "no-large-client-components",
        "type": "file-metric",
        "max_lines": 200,
        "glob": "**/*client*.tsx",
        "severity": "advisory"
...
- 동일 토폴로지 ID면 사용자 정의가 **merge** (기본 위에 override)
...
review-check은 기존 센서와 동일한 `SENSOR_RUN → FIX` 루프를 탄다. 최대 교정 횟수: 기존 정책(3회) 공유.

### 3.3 review-check 센서 검사 항목

#### Computational 검사 (결정적, LLM 없음)
...
#### Inferential 미니 리뷰 (LLM, 경량)

| 검사 | 소스 | 비용 제어 |
|------|------|-----------|
| 슬라이스 diff 컨벤션 체크 | `rules.yaml` + 템플릿 `guides.phase3` | diff만 전달, 단일 프롬프트, 짧은 응답 강제 |

- inferential 리뷰는 **Advisory** — 차단하지 않고 피드백만 제공
- 에이전트가 판단하여 수정 여부 결정
...
| fitness.json 없음 | review-check 전체 **not_applicable** (skip) |
...
- **Phase 1 연동** — deep-work가 Phase 1 Research에서 harnessability 평가 로직을 자체적으로 실행. deep-dashboard의 lib 코드를 복사하지 않고, deep-work에 경량 harnessability-runner를 두어 동일 알고리즘을 구현. 두 구현의 일관성은 테스트로 보장
...
- 다른 플러그인의 존재를 **모름** — 파일시스템에서 데이터를 읽을 뿐

### 5.3 데이터 수집

각 플러그인의 기존 출력물을 **읽기만**. 새로운 데이터를 생성하지 않음:

| 소스 | 위치 | 수집 데이터 |
|------|------|-------------|
| deep-work receipts | `.deep-work/receipts/*.json` | sensor_results, mutation_testing, health_report, fitness_delta |
| deep-review receipts | `.deep-review/receipts/*.json` | review findings, fitness 소비 결과 |
| deep-docs scan | `.deep-docs/scan-results.json` | 문서 freshness, dead references |
| Harnessability (#7) | 실행 시 계산 | 6차원 점수 |
| Fitness status | `.deep-review/fitness.json` + 최근 검증 결과 | 규칙별 pass/fail |
...
║ Top action: fix 2 dependency vulnerabilities         ║
╚══════════════════════════════════════════════════════╝
...
effectiveness = weighted_avg(
  health_status    × 0.25,  // drift 센서 결과 (clean 비율)
  fitness_status   × 0.25,  // 규칙 준수율
  session_quality  × 0.20,  // 최근 3세션 평균 quality score
  harnessability   × 0.15,  // #7 점수
  plugin_coverage  × 0.15   // 플러그인 활성 비율
)
```
