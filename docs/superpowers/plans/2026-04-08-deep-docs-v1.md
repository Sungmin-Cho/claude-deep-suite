...
결과를 JSON 형태로 반환하여 garden과 audit에서 활용 가능하게 함.
```
...
1. 이전 scan 결과가 있으면 재사용, 없으면 scan 먼저 실행
...
2. audit-metrics.md 기준으로 4개 지표 계산:

   a. **파일 크기**: 각 문서의 라인 수 확인
      ```bash
      wc -l CLAUDE.md AGENTS.md README.md docs/**/*.md
      ```
