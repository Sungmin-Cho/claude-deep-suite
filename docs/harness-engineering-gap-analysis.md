...
│  제어)      │ ✅ topology templates   │ ✅ deep-wiki 지식 베이스 │
│             │   (phase1/3/4 guides)   │ ⛔ MCP 기반 지식 검색    │
│             │ ⛔ LSP/Language Server  │ ⛔ 컨텍스트별 컨벤션 스킬│
│             │ ⛔ codemods             │                          │
├─────────────┼─────────────────────────┼──────────────────────────┤
...
**2026-04-09 업데이트 3:** Harness Templates + Self-Correction Loop + Harnessability 진단 + 통합 대시보드 구현 완료 — topology 기반 guides 자동 주입(Computational Guides), review-check 센서(Computational Sensors), 6차원 harnessability 스코어(17개 감지기), 4차원 effectiveness 대시보드. 8개 약점 모두 해소.

---

## 3대 규제 카테고리별 진단

### Maintainability Harness — 8/10 (개선됨, 2026-04-09)

**있음**: SOLID 리뷰, Insight 분석, 엔트로피 스캔, TDD 강제, deep-docs, **review-check 센서(linter/타입 체크 결과 + topology guides 기반 스타일 위반 감지)**, **dead-export 스캔(데드 코드 감지)**

**없음** (향후):
- 순환 복잡도 임계값 강제 (보고만 하고 차단 안 함)
- 테스트 커버리지 품질 검증 (mutation score로 부분 보완됨)

### Architecture Fitness Harness — 8/10 (개선됨, 2026-04-09)

**있음**: Drift Check, deep-review "architecture fit" 기준, **fitness.json 선언적 규칙 + 계산적 검증**, **file-metric (줄 수 제한)**, **forbidden-pattern (금지 패턴)**, **structure (colocated 테스트)**, **dependency (순환 의존성, dep-cruiser)**, **fitness-generator (자동 생성 제안)**, **Phase 4 Fitness Delta Gate**, **Health Required Gate (required_missing 전파)**, **topology별 fitness_defaults (템플릿 seed)**, **review-check 센서(fitness.json 규칙 기반)**
...
| **Continuous** | **양호 (개선)** | ✅ 세션 간 드리프트 감지(dead-export, stale-config, dep-vuln, coverage-trend). ✅ harnessability 스코어(6차원, 17개 감지기). ✅ 통합 effectiveness 대시보드(4차원). ⛔ 런타임 피드백, 로그 이상 탐지 전무 |
...
| 신뢰도 | 높음 | 확률적 |
| deep-suite 활용 | ✅ 충분 (linter, typecheck, coverage, mutation, topology guides, review-check, harnessability) | 적절 |
...
| 5 | ~~Harness Templates 부재~~ → **✅ 구현 완료** (topology 감지 + guides/sensors/fitness_defaults 번들) | Harness Templates | ✅ Resolved |
| 6 | ~~Self-Correction Loop 미통합~~ → **✅ 구현 완료** (review-check 센서, lint→typecheck→review-check 파이프라인) | Steering Loop | ✅ Resolved |
| 7 | ~~Harnessability 진단 부재~~ → **✅ 구현 완료** (6차원 스코어, 17개 감지기, /deep-harnessability) | Harnessability | ✅ Resolved |
| 8 | ~~통합 대시보드 부재~~ → **✅ 구현 완료** (4차원 effectiveness, action-router, /deep-harness-dashboard) | Human Steering | ✅ Resolved |
