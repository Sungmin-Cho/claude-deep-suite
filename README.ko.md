...
| [deep-wiki](https://github.com/Sungmin-Cho/claude-deep-wiki) | 1.4.2 | v1.4.2 — F1 disk-authoritative recovery + F2/B3/I2 백로그 + 3-round /deep-review fixups (13 critical + 4 warning) |
...
- **F1 disk-authoritative recovery + F2/B3/I2 백로그 + 3-round /deep-review fixups** (v1.4.2) — `docs/handoff-2026-05-06-v1.4.2.md`의 4개 v1.4.1 백로그를 닫는 패치 릴리즈 + 3 round /deep-review 교차 검증 (3-way Opus + Codex review + Codex adversarial; 누적 13 critical + 4 warning 모두 ACCEPT per receiving-review skill protocol; non-convergence 패턴으로 fix-and-go cap 발동 — 4th round 미실행, Opus stopping rule). **F1 (HIGH)** — synthesizer `existing_page_body` truncation으로 인한 C3 false-positive abort: v1.4.1 cache-active dogfood에서 Stage 1 LLM이 극심하게 truncate된 bytes emit 관측 (12377B disk → 725B emit) → main hash baseline이 항상 disk와 다름 → C3가 항상 "concurrent ingest detected" → 모든 update abort. Fix: main이 Stage 1 return 후 disk에서 페이지를 다시 읽어 disk bytes를 C3 hash baseline AND Stage 2 worker / inline-write synthesis context로 통일 (이전 P6 round-1 hash-from-emit compute 흡수). Hash 값은 v1.4.1과 byte-identical 아님 (`$(cat)` newline-strip vs v1.4.1 `printf '%s' "$emit"`) 그러나 Stage 3 success/abort 결정은 spec-compliant agent에서 동등 (F1 capture와 C3 re-check의 byte-stripping symmetric). single-source 경로만 적용. **F2 (MEDIUM)** — §3.9 4번째 invocation site: pre-v1.4.2의 dirty-scan brackets는 3개 site (multi-source A4 fanout, Case B2 second-pass, A5 single-source fanout)에서만 작동했고 single-source Stage 1 analysis dispatch는 미커버. v1.4.2는 label `"A5-analysis"` invocation을 `invoke deep-wiki:wiki-synthesizer-analysis` 직접 주변에 추가하여 misresolved/downgraded analysis subagent의 `<wiki_root>/`-internal mutation을 downstream 분기와 무관하게 catch. **B3 (MEDIUM)** — `phase_timing_ms` schema-additive field on `ingest` log lines (`{stage_1_analysis, stage_2_fanout, stage_3_write, total}` ms 정수); `_ts_ms` Bash 3.2 portable helper (python3 → `date +%s000` 폴백). 프로덕션 비용 ingest당 ~12ms. v1.4.0 plan §10.2 deferred. **I2 (LOW)** — V-2/V-3 WebFetch probe full-URL allowlist 비교: stub server가 Host header를 6번째 TSV 컬럼으로 기록; `v2-v3-record.sh`가 `<host><path>?<query>`로 정규화된 비교를 위해 추출. cross-host + query-string exfil false-pass 차단 (pre-v1.4.2 path-only 비교는 `attacker.com/feed`를 허용된 `example.com/feed`와 일치 처리). backward-compatible: pre-v1.4.2 5-column 로그는 `lossy-pre-v1.4.2-log` annotation 트리거. 테스트 인프라 한정. **3-round /deep-review fixups** (13 critical + 4 warning): Round 1 (post-impl drift) — F1.1 sub-threshold drift→A5 fanout escalate (silent corruption 차단; 2/3 agreement), F1.2 Step 7.6.C entry PARTIAL_FAIL 보존 (sentinel-skip 차단; 2/3 agreement), F1.3 basename pre-disk-read regex 가드 (path traversal 차단), B3.1 path-coverage matrix vs Step 10 omission rule 정합 (4-column 표), F1.4+F1.5 byte-identical claim 제거 + agent doc Rule 4 정확성 (synth bytes → telemetry-only), W3 set -u defaultization, W4 v2-v3-record empty-log short-circuit. Round 2 (regression) — R2.F1.6 hash-compare로 size-delta heuristic 교체 (concurrent-ingest baseline race T0→T1 window 차단; 2/3 agreement), R2.F1.7 all-dropped → 신규 `do_all_failed_under_lock` (Step 7.5.B; terminal-skip bypass 차단; 2/3 agreement), R2.C1 CLAUDE.md byte-identical claim drift 정정. Round 3 (focused on novel `do_all_failed_under_lock`) — R3.P2.1 slug-vs-descriptor mismatch (caller가 `${SOURCES[0]%%|*}` 추출; 2/3 agreement), R3.P2.2 3-strike retry counter Step 7.5.M-D mirror (count≥3 시 `ingest-fail` emit + `.pending-scan` promote; 2/3 agreement), R3.C-1 mkdir soft-fail + `trap - EXIT` 정상화, R3.C-2 first-ingest baseline yaml R4-Adv-Adv-2 mirror, R3.C-3 `FAILED_PAGE_FILES` parallel array init + 3 F1 site에 push, R3.W-1 stage_3_write timing scope를 `STAGE_3_START_MS_FAIL`로 한정, R3.W-2 reason taxonomy 정규화 (`all_f1_dropped` → `all f1 dropped`). **Mandatory verbatim L1+L2 (v1.4.1 §11.5에서 계승)**: V-0 best-effort + §3.9 in-root scope only — 보존. v1.4.2 L1 addendum: I2 fix가 probe fidelity 개선했으나 v1.4.1 final agents 대상 empirical V-2/V-3 SECOND run은 보류 (sandbox orchestration 워크스트림). Process-level sandboxing → v1.5.0+. **하위 호환성**: F1은 single-source 경로만 적용; multi-source A4 (worker, analysis-mode 아님)는 항목에 `existing_body_hash` 필드 없음 — 영향 없음. v1.4.1 대비 외부 API 변경 없음.
