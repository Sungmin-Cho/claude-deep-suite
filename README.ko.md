...
- **`existing_body_hash` disk-authoritative 읽기 + Stage 1 dispatch 커버리지 + `phase_timing_ms` 텔레메트리** (v1.4.2) — 4-item v1.4.1 backlog 닫기 + 2-round /deep-review post-impl fixups (3 rounds, 13 critical + 4 warning 모두 ACCEPT) 후 patch 릴리즈. **F1 (HIGH)**: main이 Stage 1 return 후 disk에서 페이지 bytes를 다시 읽음 — synthesizer `existing_page_body` 잘림으로 인한 C3 false-positive abort 차단 (v1.4.1 cache-active dogfood에서 12377B disk → 725B emit 등 발견). **F2 (MEDIUM)**: single-source Stage 1 dispatch §3.9 4번째 invocation site (`A5-analysis`) 추가 — v1.4.1 3-site bracket (A4-fanout / A4-second-pass / A5-fanout) gap 닫음. **B3 (MEDIUM)**: `log.jsonl`의 `ingest` 라인에 `phase_timing_ms: {stage_1_analysis, stage_2_fanout, stage_3_write, total}` ms-integer 텔레메트리 (schema-additive — wiki-lint Step 6 LOG-INVARIANT scan 영향 없음; `_ts_ms` Bash 3.2 portable helper, python3 → `date +%s000` fallback). **Post-impl fixups**: F1.1 sub-threshold drift → A5 fanout escalate (silent corruption 차단), F1.2 Step 7.6.C entry에서 PARTIAL_FAIL 보존, F1.3 basename regex pre-disk-read 가드 (path-traversal 차단), R2.F1.6 hash-compare가 size-delta 대체 (concurrent-ingest baseline race 차단), R2.F1.7 all-dropped → `do_all_failed_under_lock` (terminal-skip bypass 차단 — partial_fail sentinel 항상 작성), R3.P2.1 slug-vs-descriptor mismatch 차단 (`slug="${SOURCES[0]%%|*}"`), R3.P2.2 single-source F1 all-fail 3-strike retry counter (Step 7.5.M-D mirror). L1+L2 mandatory verbatim language v1.4.1 §11.5에서 계승. 외부 API 변경 없음.
- **M3 envelope 채택** (v1.5.0) — Minor 릴리즈; deep-wiki는 M3 Phase 2 plugin migration의 **6번째(마지막)** — 본 머지 후 suite-side Phase 3 (marketplace.json SHA bump + payload-registry schema 교체 + adoption ledger) 진행 가능. `<wiki_root>/.wiki-meta/index.json`이 M3 cross-plugin envelope (`docs/envelope-migration.md` §1)로 감싸짐; legacy `{pages, generated_at}` 구조는 `payload` 안에 그대로 보존. Consumer는 envelope-aware reader를 통해 legacy shape를 그대로 받음 (v1.5.0+ envelope-wrapped + pre-1.5.0 legacy 양쪽 통과). **새 helpers**: `hooks/scripts/envelope.js` (zero-dep — ULID Crockford Base32 MSB-first generator, `detectGit`, `loadProducerVersion`, `wrapEnvelope`/`unwrapEnvelope`, `isValidEnvelope`; producer version은 `.claude-plugin/plugin.json`에서 literal-cwd-resolve로 읽음), `wrap-index-envelope.js` (atomic temp+rename writer; `--source-page` 반복형 path-only flag + `--source-artifact path[:run_id]` self-consistency auto-harvest), `read-index-envelope.js` (envelope+legacy reader — input 형태와 무관하게 stdout에 legacy shape 출력); `scripts/validate-envelope-emit.js` (release-lint mirror — `additionalProperties:false` strict on every nested object, ULID/SemVer 2.0.0/RFC 3339/kebab-case regex, identity check + corrupt-payload defense). **Identity 계약**: `unwrapEnvelope()`의 3-way guard — `producer === "deep-wiki"`, `artifact_kind === "index"`, `schema.name === "index"`; `producer_version`은 `plugin.json.version` 미러 (single source of truth); `run_id`는 26-char Crockford Base32 ULID (lex sort = time sort). **다중 소스 aggregator**: `parent_run_id` 기본 omit (index.json은 모든 페이지 frontmatter scan 결과 — single parent artifact 없음); 페이지는 `provenance.source_artifacts[]`에 path-only 기록 (markdown — envelope detect 불가). 5개 명령 (`wiki-setup`, `wiki-rebuild`, `wiki-ingest`, `wiki-query`, `wiki-lint`) 모두 envelope-aware reader/writer로 리팩터; in-memory merge는 unwrapped payload 위에서 동작하므로 기존 jq pipeline (`.pages[].file`, `.generated_at`) 수정 없이 동작. `tests/envelope-{emit,chain}.test.js`에 87+ tests (wrap roundtrip, identity gates, parseArgs boundary scalar+repeatable 빈 값 거부, atomic-write residue check, envelope-aware reader legacy pass-through, multi-source aggregator 계약 — `parent_run_id` 부재, `source_artifacts` path-only). 하위 호환 — pre-1.5.0 legacy `index.json`은 reader graceful-passthrough; 업그레이드 후 `/wiki-rebuild` 불필요 (legacy 파일은 다음 `/wiki-rebuild` 호출 시 envelope 재포장, 데이터 변경 없음).
- **M5.5 #3 scan-vault-changes hook golden test** (v1.5.1) — Patch (tests-only, 프로덕션 코드 변경 없음). `hooks/scripts/scan-vault-changes.sh` (SessionStart 자동 ingest 감지) 계약 — detected-count / 파일 리스트 / exit-code / `.pending-scan` 보존 — fixture corpus로 pin해 `npm test`로 회귀 차단. 새 `hooks/scripts/test-helpers/run-scan-vault.js` (hermetic helper: `scrubHostEnv()`가 HOME / CLAUDE_PROJECT_DIR / DEEP_WIKI_ROOT 제거 — 개발자의 실제 `~/.claude/deep-wiki-config.yaml` + vault가 테스트 프로세스에 leak 방지; `runScanVault()`가 `HOME=tmpRoot`로 hook spawn + tmpRoot/.claude/에 yaml materialize; `parseHookOutput()`가 stdout에서 한국어 헤더 카운트 + 파일 라인 추출). 새 `tests/auto-ingest-golden.test.js` (node:test driver — `tests/fixtures/golden/<name>.{input,expected}.json` pair 발견, half-commit loud-fail, 각 fixture의 `vault_tree`를 tmpdir에 materialize, `mtime_offsets`를 `fs.utimesSync`로 적용 — deterministic mtime-comparison test, `.last-scan` / `.pending-scan` 요청 시 pre-seed → exit code + 헤더 카운트 + expected/forbidden 파일 셋 + `pending_scan_preserved` assert). 새 `tests/fixtures/golden/` 8-시나리오 corpus: 빈 vault → silent exit, 3 new .md → 전부 detect, `.obsidian/` + `.trash/` 제외 (find prune), `.last-scan`보다 오래된 파일 필터 (mtime branch), `auto_ingest.require_tag: project` 필터, `auto_ingest.ignore_globs: [archive/**]` 필터, `deep-wiki-config.yaml` 부재 → silent exit (CONFIG-missing branch), valid `.pending-scan` → verbatim 보존 (advance 안 함). `claude-deep-work` PR #29 phase-guard golden 패턴 미러; scan-vault deviation — free-form 한국어 systemMessage stdout (JSON `{decision, reason}` 아님)이라 helper가 header-count regex + 파일 라인 parse; `${VAULT_ROOT}` / `${WIKI_ROOT}` template substitution + `mtime_offsets` 추가 (vault-fixture mtime control이 이 hook intrinsic). Obsidian-cli supplement (`obsidian recents`) 분기는 자연 skip (helper가 PATH에 obsidian 미배치). Test count 111 → 119 (+8 fixtures).
- **M5.5 #5 `.pending-scan` stale-recovery integration test** (v1.5.2) — Patch (tests-only, 프로덕션 코드 변경 없음). `hooks/scripts/scan-vault-changes.sh`의 `.pending-scan` recovery 계약을 인위적으로 dangle된 state에 대해 pin. Hermetic via `HOME=tmpRoot` + tmpRoot config — 개발자의 실제 `~/.claude/deep-wiki-config.yaml` / 실 vault 미접근. **7 cases (A-G)**: A) invalid `.pending-scan` (non-ISO-8601) → fresh ISO-8601 timestamp overwrite + no-crash. B) valid `.pending-scan` → **verbatim 보존** across hook fires (H1 regression guard from ultrareview bug_006 — every-fire overwrite 시 oldest-detection-window lower bound 소실). C) dangling `.pending-scan` (`.last-scan`보다 older) → 양쪽 preserved + no-crash (wiki-lint Step 11 State B target). D) `.last-scan` 부재 + valid `.pending-scan` → pending이 `LAST_SCAN`으로 사용 + preserved. E) fresh install (둘 다 부재) → 현재 ISO-8601 timestamp로 `.pending-scan` 생성. F) empty `.pending-scan` (truncate crash mid-write 시뮬) → valid timestamp overwrite. G) corrupt UTF-8 → cleanly overwritten, no non-printable bytes 잔존. `wiki-lint.md` Step 11 / 12 stale-detection-and-fix protocol (Claude follows markdown protocol — directly testable 아님)의 executable companion. Test count 119 → 126 (+7). 또한 post-impl docs sync로 CLAUDE.md + wiki-schema 스킬 drift도 닫음 (plugin-validator + skill-reviewer 검증 결과 대응).
- **wiki-schema 스킬 description 메타데이터 수정** (v1.5.3) — Patch (메타데이터 전용, 프로덕션 코드 변경 없음). `skills/wiki-schema/SKILL.md` frontmatter `description`이 1077자로 Claude Code 1024자 제한 초과; deep-suite 마켓플레이스 캐시에서 플러그인 로드 시 `invalid description: exceeds maximum length of 1024 characters` 경고 발생. verbose phrasing 축약 ("This skill defines …" → "Defines …", "It should be activated whenever …" → "Activate whenever …", slash command 별 부연 산문 압축)하되 모든 trigger 키워드 (5개 slash command, 10개 lifecycle action, 4개 critical invariant, 모든 storage-layout 경로) 보존. 새 길이: 978자. 프로덕션 코드, 테스트, hook, 에이전트, 명령, 스킬 본문 모두 변경 없음. 테스트 수 동일 (126).
