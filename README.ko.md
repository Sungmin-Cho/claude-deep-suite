...
구조화된 개발, 지식 관리, 자율 실험, 독립 코드 리뷰, 문서 가드닝, 하네스 진단을 위한 6개 플러그인을 묶어 제공하는 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 플러그인 마켓플레이스.
...
| 플러그인 | 버전 | 설명 |
|---|---|---|
| [deep-work](https://github.com/Sungmin-Cho/claude-deep-work) | 6.7.0 | Evidence-Driven Development Protocol |
| [deep-wiki](https://github.com/Sungmin-Cho/claude-deep-wiki) | 1.6.1 | LLM-managed markdown wiki |
| [deep-evolve](https://github.com/Sungmin-Cho/claude-deep-evolve) | 3.4.1 | Autonomous Experimentation Protocol |
...
각 플러그인은 별도 Git 저장소: `github.com/Sungmin-Cho/claude-deep-{name}`.
...
`/deep-work "task"` 하나로 **Brainstorm → Research → Plan → Implement → Test → Integrate** 전체 파이프라인을 자동 진행. Phase 3 외 코드 파일 수정은 hook으로 물리적 차단.
...
RAG처럼 매번 지식을 다시 발견하지 않고, Claude Code가 영속 wiki를 점진적으로 구축·유지. 새 소스를 추가하면 LLM이 읽고 핵심 정보를 추출해 기존 wiki에 통합. 지식이 시간이 갈수록 누적된다.
...
- **Auto-ingest 훅** — `SessionStart` hook이 vault 수정 `.md` 감지 후 `/wiki-ingest` 자동 트리거; `auto_ingest:` config로 opt-in
...
**Independent Evaluator** — AI 코딩 에이전트 출력을 별도 Opus subagent로 리뷰. Codex 플러그인이 설치되어 있으면 3-way cross-model 검증.
...
- **Independent evaluator** — Generator context 없는 별도 Opus subagent
...
**Document Gardening Agent** — `CLAUDE.md`, `AGENTS.md` 같은 에이전트 지시 문서의 freshness 검증 + 자동 수리.
...
| 명령 | 설명 |
|---|---|
| `/deep-docs scan` | stale 참조, 이동 경로, outdated 예제 감지 |
| `/deep-docs garden` | 사용자 확인 후 자동 수리 |
...
Claude Code 공식 `marketplace.json` 스키마에 들어가지 않는 cross-plugin 메타데이터는 사이드카 `.claude-plugin/suite-extensions.json`에 두며, [`schemas/suite-extensions.schema.json`](./schemas/suite-extensions.schema.json) (JSON Schema Draft 2020-12)으로 검증.
