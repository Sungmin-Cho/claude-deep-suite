...
Each plugin lives in its own Git repository: `github.com/Sungmin-Cho/deep-{name}`. The marketplace **name** remains `claude-deep-suite` so existing installs keep their plugin keys; Codex support is exposed through the Codex marketplace mirror and each plugin's `.codex-plugin/plugin.json`.
