...
| [deep-wiki](https://github.com/Sungmin-Cho/claude-deep-wiki) | 1.4.2 | v1.4.2 — F1 disk-authoritative recovery + F2/B3/I2 backlog closure + 3-round /deep-review fixups (13 critical + 4 warning) |
...
- **F1 disk-authoritative recovery + F2/B3/I2 backlog closure + 3-round /deep-review fixups** (v1.4.2) — Patch release closing the 4-item v1.4.1 backlog from `docs/handoff-2026-05-06-v1.4.2.md` plus 3 rounds of `/deep-review` cross-validation (3-way Opus + Codex review + Codex adversarial; cumulative 13 critical + 4 warning findings, all ACCEPT per receiving-review skill protocol; non-convergence pattern triggered fix-and-go cap per Opus stopping rule — no 4th round). **F1 (HIGH)** — synthesizer `existing_page_body` truncation false-positive C3 abort: v1.4.1 cache-active dogfood observed Stage 1 LLM emit dramatically-truncated bytes (12377B disk → 725B emit), so main's hash baseline always differed from disk → C3 always reported "concurrent ingest detected" → every update aborted. Fix: main re-reads pages from DISK after Stage 1 returns and uses disk bytes as both the C3 hash baseline AND Stage 2 worker / inline-write synthesis context (subsumes prior P6 round-1 hash-from-emit compute). Hash values are NOT byte-identical to v1.4.1 (`$(cat)` newline-strip vs v1.4.1's `printf '%s' "$emit"`) but Stage 3 success/abort decisions remain equivalent for spec-compliant agents (symmetric byte-stripping across F1 capture and C3 re-check). Single-source path only. **F2 (MEDIUM)** — §3.9 4th invocation site: pre-v1.4.2 dirty-scan brackets fired at 3 dispatch sites (multi-source A4 fanout, Case B2 second-pass, A5 single-source fanout) but did NOT bracket single-source Stage 1 analysis dispatch. v1.4.2 adds label `"A5-analysis"` invocation immediately around `invoke deep-wiki:wiki-synthesizer-analysis` so a misresolved/downgraded analysis subagent's `<wiki_root>/`-internal mutations are caught regardless of downstream branch. **B3 (MEDIUM)** — `phase_timing_ms` schema-additive field on `ingest` log lines (`{stage_1_analysis, stage_2_fanout, stage_3_write, total}` ms ints); `_ts_ms` Bash 3.2 portable helper (python3 → `date +%s000` fallback). Production cost ~12ms per ingest. v1.4.0 plan §10.2 deferred item. **I2 (LOW)** — V-2/V-3 WebFetch probe full-URL allowlist comparison: stub server records Host header as 6th TSV column; `v2-v3-record.sh` extracts `<host><path>?<query>` for normalized comparison. Closes cross-host + query-string exfil false-pass (pre-v1.4.2 path-only comparison matched `attacker.com/feed` against allowed `example.com/feed`). Backward-compatible: pre-v1.4.2 5-column logs trigger `lossy-pre-v1.4.2-log` annotation. Testing infrastructure only. **3-round /deep-review fixups** (13 critical + 4 warning):  Round 1 (post-impl drift) — F1.1 sub-threshold drift→A5 fanout escalate (silent corruption block; 2/3 agreement), F1.2 Step 7.6.C entry PARTIAL_FAIL preservation (sentinel-skip prevention; 2/3 agreement), F1.3 basename pre-disk-read regex guard (path traversal protection), B3.1 path-coverage matrix vs Step 10 omission rule alignment (4-column table), F1.4+F1.5 byte-identical claim removal + agent doc Rule 4 precision (synth bytes → telemetry-only), W3 set -u defaultization, W4 v2-v3-record empty-log short-circuit. Round 2 (regression) — R2.F1.6 hash-compare replaces size-delta heuristic (concurrent-ingest baseline race close in T0→T1 window; 2/3 agreement), R2.F1.7 all-dropped → new `do_all_failed_under_lock` (Step 7.5.B; terminal-skip bypass close; 2/3 agreement), R2.C1 CLAUDE.md byte-identical claim drift fix. Round 3 (focused on novel `do_all_failed_under_lock`) — R3.P2.1 slug-vs-descriptor mismatch (caller now extracts via `${SOURCES[0]%%|*}`; 2/3 agreement), R3.P2.2 3-strike retry counter mirror from Step 7.5.M-D (count≥3 → emit `ingest-fail` + promote `.pending-scan`; 2/3 agreement), R3.C-1 mkdir soft-fail + `trap - EXIT` restoration, R3.C-2 first-ingest baseline yaml materialization R4-Adv-Adv-2 mirror, R3.C-3 `FAILED_PAGE_FILES` parallel array init + push at 3 F1 sites, R3.W-1 stage_3_write timing scope via `STAGE_3_START_MS_FAIL`, R3.W-2 reason taxonomy normalization (`all_f1_dropped` → `all f1 dropped`). **Mandatory verbatim L1+L2 (carried from v1.4.1 §11.5)**: V-0 best-effort + §3.9 in-root scope only — preserved. v1.4.2 addendum to L1: I2 fix improves probe fidelity but empirical V-2/V-3 SECOND run vs v1.4.1 final agents remains deferred (sandbox orchestration workstream). Process-level sandboxing → v1.5.0+. **Backward compat**: F1 single-source path only; multi-source A4 (workers, not analysis-mode) has no `existing_body_hash` field on entries — unaffected. No external API changes from v1.4.1.
