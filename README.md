...
| [deep-review](https://github.com/Sungmin-Cho/claude-deep-review) | 1.5.1 | Independent Evaluator |
| [deep-docs](https://github.com/Sungmin-Cho/claude-deep-docs) | 1.2.1 | Document gardening agent |
| [deep-dashboard](https://github.com/Sungmin-Cho/claude-deep-dashboard) | 1.3.4 | Cross-plugin harness diagnostics + suite telemetry |
...
- **`existing_body_hash` disk-authoritative read + Stage 1 dispatch coverage + `phase_timing_ms` telemetry** (v1.4.2) — Patch closing the 4-item v1.4.1 backlog plus 2-round /deep-review post-impl fixups (3 rounds, 13 critical + 4 warning all ACCEPT). **F1 (HIGH)**: main re-reads page bytes from disk after Stage 1 return — closes C3 false-positive abort from synthesizer `existing_page_body` truncation (v1.4.1 cache-active dogfood found 12377B disk → 725B emit cases). **F2 (MEDIUM)**: single-source Stage 1 dispatch §3.9 4th invocation site (`A5-analysis`) added — v1.4.1 3-site bracket (A4-fanout / A4-second-pass / A5-fanout) gap closed. **B3 (MEDIUM)**: `phase_timing_ms: {stage_1_analysis, stage_2_fanout, stage_3_write, total}` ms-integer telemetry on `log.jsonl` `ingest` lines (schema-additive — wiki-lint Step 6 LOG-INVARIANT scan unaffected; `_ts_ms` Bash 3.2 portable helper via python3 → `date +%s000` fallback). **Post-impl fixups**: F1.1 sub-threshold drift → A5 fanout escalate (silent corruption blocked), F1.2 PARTIAL_FAIL preservation at Step 7.6.C entry, F1.3 basename regex pre-disk-read guard (path-traversal blocked), R2.F1.6 hash-compare replaces size-delta (concurrent-ingest baseline race blocked), R2.F1.7 all-dropped → `do_all_failed_under_lock` (terminal-skip bypass blocked — partial_fail sentinel always written), R3.P2.1 slug-vs-descriptor mismatch blocked (`slug="${SOURCES[0]%%|*}"`), R3.P2.2 single-source F1 all-fail 3-strike retry counter (Step 7.5.M-D mirror). L1+L2 mandatory verbatim language inherited from v1.4.1 §11.5. No external API change.
- **M3 envelope adoption** (v1.5.0) — Minor release; deep-wiki is the **6th and final** M3 Phase 2 plugin migration (suite-side Phase 3 — marketplace.json SHA bump + payload-registry schema replacement + adoption ledger — unblocked after this). `<wiki_root>/.wiki-meta/index.json` is wrapped in the M3 cross-plugin envelope (cf. `docs/envelope-migration.md` §1); legacy `{pages, generated_at}` shape is preserved verbatim inside `payload`. Consumers use the envelope-aware reader to get the legacy shape regardless of whether the file is v1.5.0+ envelope-wrapped or pre-1.5.0 legacy. **New helpers**: `hooks/scripts/envelope.js` (shared zero-dep — ULID Crockford Base32 MSB-first generator, `detectGit`, `loadProducerVersion`, `wrapEnvelope`/`unwrapEnvelope`, `isValidEnvelope`; producer version read from `.claude-plugin/plugin.json` via literal-cwd-resolve), `wrap-index-envelope.js` (atomic temp+rename writer with `--source-page` repeatable path-only flag and `--source-artifact path[:run_id]` self-consistency auto-harvest), `read-index-envelope.js` (envelope+legacy reader emitting legacy shape on stdout regardless of input form); `scripts/validate-envelope-emit.js` (release-lint mirror of the suite envelope schema — `additionalProperties:false` strict on every nested object, ULID/SemVer 2.0.0/RFC 3339/kebab-case regex, identity check + corrupt-payload defense). **Identity contract**: 3-way guard at `unwrapEnvelope()` — `producer === "deep-wiki"`, `artifact_kind === "index"`, `schema.name === "index"`; `producer_version` mirrors `plugin.json.version` (single source of truth); `run_id` is a 26-char Crockford Base32 ULID (lex sort = time sort). **Multi-source aggregator**: `parent_run_id` omitted by default (index.json is built by scanning every page's frontmatter — no single source artifact acts as parent); pages recorded in `provenance.source_artifacts[]` path-only (markdown — no envelope detect). All 5 commands (`wiki-setup`, `wiki-rebuild`, `wiki-ingest`, `wiki-query`, `wiki-lint`) refactored to envelope-aware reader/writer paths; in-memory merge operates on the unwrapped payload so existing jq pipelines (`.pages[].file`, `.generated_at`) work without modification. 87+ tests across `tests/envelope-{emit,chain}.test.js` (wrap roundtrip, identity gates, parseArgs boundary scalar+repeatable empty-value rejection, atomic-write residue check, envelope-aware reader legacy pass-through, multi-source aggregator contract — no `parent_run_id`, `source_artifacts` path-only). Backward-compatible — pre-1.5.0 legacy `index.json` passes through the reader graceful-passthrough; users do not need to run `/wiki-rebuild` after upgrading (existing legacy files re-wrapped on the next `/wiki-rebuild` invocation, data unchanged).
- **M5.5 #3 scan-vault-changes hook golden test** (v1.5.1) — Patch (tests-only, no production code change). Pins `hooks/scripts/scan-vault-changes.sh` (SessionStart auto-ingest detection) contract — detected-count, file-list, exit-code, `.pending-scan` preservation — against a fixture corpus so regressions are caught by `npm test`. New `hooks/scripts/test-helpers/run-scan-vault.js` (hermetic helper: `scrubHostEnv()` removes HOME / CLAUDE_PROJECT_DIR / DEEP_WIKI_ROOT so the developer's real `~/.claude/deep-wiki-config.yaml` + vault cannot leak into the test process; `runScanVault()` spawns the hook with `HOME=tmpRoot` and materializes the YAML config inside `tmpRoot/.claude/`; `parseHookOutput()` extracts the Korean header count + file lines from stdout). New `tests/auto-ingest-golden.test.js` (node:test driver — discovers `tests/fixtures/golden/<name>.{input,expected}.json` pairs, fails loud on half-commits, materializes each fixture's `vault_tree` into a tmpdir, applies `mtime_offsets` via `fs.utimesSync` for deterministic mtime-comparison tests, pre-seeds `.last-scan` / `.pending-scan` when requested, then asserts exit code + header count + expected/forbidden file sets + `pending_scan_preserved`). New `tests/fixtures/golden/` 8-scenario corpus: empty vault → silent exit, 3 new .md → all detected, `.obsidian/` + `.trash/` excluded (find prune), files older than `.last-scan` filtered (mtime branch), `auto_ingest.require_tag: project` filter, `auto_ingest.ignore_globs: [archive/**]` filter, no `deep-wiki-config.yaml` → silent exit (CONFIG-missing branch), valid existing `.pending-scan` preserved verbatim (not advanced). Pattern mirrors `claude-deep-work` PR #29 phase-guard golden; scan-vault deviation — free-form Korean systemMessage stdout (not JSON `{decision, reason}`), so the helper uses header-count regex + file-list line parsing; `${VAULT_ROOT}` / `${WIKI_ROOT}` template substitution and `mtime_offsets` added because vault-fixture mtime control is intrinsic to this hook. Obsidian-cli supplement branch (`obsidian recents`) naturally skipped (helper does not put `obsidian` on PATH). Test count 111 → 119 (+8 fixtures).
- **M5.5 #5 `.pending-scan` stale-recovery integration test** (v1.5.2) — Patch (tests-only, no production code change). Pins `hooks/scripts/scan-vault-changes.sh`'s `.pending-scan` recovery contract against artificially-dangled state. Hermetic via `HOME=tmpRoot` + tmpRoot config — never touches the developer's real `~/.claude/deep-wiki-config.yaml` or real vault. **7 cases (A-G)**: A) invalid `.pending-scan` (non-ISO-8601) → overwritten with fresh ISO-8601 timestamp + no-crash. B) valid `.pending-scan` → **preserved verbatim** across hook fires (H1 regression guard from ultrareview bug_006 — every-fire overwrite would erase the oldest-detection-window lower bound). C) dangling `.pending-scan` older than `.last-scan` → both preserved + no-crash (wiki-lint Step 11 State B target). D) `.last-scan` absent + valid `.pending-scan` → pending used as `LAST_SCAN` + preserved. E) fresh install (neither file) → hook creates `.pending-scan` with current ISO-8601 timestamp. F) empty `.pending-scan` (truncate crash mid-write simulation) → overwritten with valid timestamp. G) corrupt UTF-8 bytes → cleanly overwritten, no non-printable bytes survive. Executable companion to `wiki-lint.md` Step 11 / 12 stale-detection-and-fix protocol (a markdown protocol Claude follows, not directly testable). Test count 119 → 126 (+7). Also closes plugin-validator + skill-reviewer drift in CLAUDE.md + wiki-schema skill via post-impl docs sync.
- **wiki-schema skill description metadata fix** (v1.5.3) — Patch (metadata-only, no production code change). `skills/wiki-schema/SKILL.md` frontmatter `description` was 1077 chars, exceeding Claude Code's 1024-character cap; surfaced as `invalid description: exceeds maximum length of 1024 characters` warning when loading the cached plugin from the deep-suite marketplace. Trimmed verbose phrasing ("This skill defines …" → "Defines …", "It should be activated whenever …" → "Activate whenever …", per-slash-command qualifier prose collapsed) while preserving every trigger keyword (all 5 slash commands, all 10 lifecycle actions, the 4 critical invariants, every storage-layout path). New length: 978 chars. Production code, tests, hooks, agents, commands, and skill body all unchanged. Test count unchanged (126).
