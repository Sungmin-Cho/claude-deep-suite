...
| deep-wiki | 1.4.2 | LLM-managed markdown wiki (v1.4.2 patch: F1 disk-authoritative recovery for synthesizer `existing_page_body` truncation false-positive C3 abort + F2 §3.9 4th invocation site (single-source Stage 1 bracketing) + B3 `phase_timing_ms` schema-additive telemetry + I2 V-2/V-3 probe full-URL allowlist comparison; 3 rounds of /deep-review fixups (13 critical + 4 warning all ACCEPT — Round 1: F1.1+F1.2+F1.3+B3.1+F1.4+F1.5+W3+W4, Round 2: R2.F1.6 hash-compare + R2.F1.7 `do_all_failed_under_lock` + R2.C1, Round 3: R3.P2.1 slug extraction + R3.P2.2 3-strike retry counter + R3.C-1/C-2/C-3/W-1/W-2). L1+L2 mandatory verbatim from v1.4.1 §11.5 carried forward. No external API changes; hash NOT byte-identical to v1.4.1 but Stage 3 decisions equivalent for spec-compliant agents) |
